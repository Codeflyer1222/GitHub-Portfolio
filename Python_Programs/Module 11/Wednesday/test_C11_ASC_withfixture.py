#!/usr/bin/env python3
#
# Problem: C11 Test class example
# Files: 
#     test_C11_ACS_withfixture.py (this file)
#     survey.py - AnonymousSurvey class definition
#     language_survey.py - main program module
#
# Author: matthes (copied from text pg. 220, 221)
# Date: 7 january 25
#
# import pytest to use fixture
import pytest

# import class to be tested
from survey import AnonymousSurvey

# define fixture function which is utilized by other test functions
#Video starts at minute 32
@pytest.fixture
def language_survey ():
    """Set up fixture."""
    question = "What langauge did you first learn to speak?"
    language_survey = AnonymousSurvey (question)
    return language_survey

# define atribute definition test function
def test_survey_creation (language_survey):
    """Test that a single response is stored properly."""
    assert language_survey.question == "What langauge did you first learn to speak?"

# define class test function
def test_store_single_response (language_survey):
    """Test that a single response is stored properly."""
    language_survey.store_response ('English')
    assert 'English' in language_survey.responses

def test_store_three_response (language_survey):
    """ the code below is for three entries """
    responses = ['English', 'Spanish', 'Mandrain']
    for response in responses:
        language_survey.store_response (response)
    
    for response in responses:
        assert response in language_survey.responses
